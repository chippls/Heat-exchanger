 function [T_h, T_c, P_h, P_c, Nu_h, Nu_c, Re_h, Re_c,T_wo,T_w,Q_h,Q_c]  = CFHX12(P_h_in,P_c_in,T_h_in, T_c_in, d1,d2,d3,d4, D_coil,cycles, m_dot, roughness,epsilon)
...
%          Nu_h(i) = Pr_h(i)*Re_h(i)*f_h(i)/(8*(1+12.7*sqrt(f_h(i)/8)*(Pr_h(i)^(2/3)-1)));
        ...
        h_correct_h(i) = Nu_h(i) * k_h / D_h_h;
...
%         h_correct_h(i) = (1+3.6*(1-D_h_h/D_coil)*(D_h_h/D_coil)^0.8)*h_h;
        ...
        c(i) = m_h*cp_h-h_correct_h(i) * deltaA_h/2;
        d(i) = m_c*cp_c-h_correct_c(i) * deltaA_c/2;
        e(i) = h_correct_h(i) * deltaA_h/2 + m_h*cp_h;
        f(i) = h_correct_c(i) * deltaA_c/2 + m_c*cp_c;
